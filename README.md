...
It does the research, cites every number, enforces a banned-words list, and explains *why* it flags each issue (Sept 2025 Quality Rater Guidelines, March 2026 core update, AI-detection classifiers, platform norms). Output is production-grade Markdown: comparison tables, ✅/❌ audits, prioritized fix lists, schema code blocks. It tells you when a draft isn't publication-ready and what a compliance officer still needs to review.
...
Google's [March 2026 core update](./skills/pubcraft/SOURCES.md#1-google--search-quality-ranking-and-updates) sharpened SpamBrain enforcement; AI content farms lost 60–90% of organic visibility. Page #1 organic CTR drops by [34.5% when an AI Overview appears](./skills/pubcraft/SOURCES.md#2-ai-search--geo--aeo--llmo) (Semrush, 10M-keyword study). Medium suspends accounts for AI-only earning. LinkedIn deboosts generic GPT prose with its [360Brew classifier](./skills/pubcraft/SOURCES.md#4-platform-algorithms--primary--official-sources). Reddit downvotes the moment "It's important to note that…" hits the page.
...
Each rule below traces to a primary source. Full bibliography in **[SOURCES.md](./skills/pubcraft/SOURCES.md)**.
...
*Why:* Sept 2025 [Quality Rater Guidelines](./skills/pubcraft/SOURCES.md#1-google--search-quality-ranking-and-updates) flag "rephrased existing content with no original information" as Lowest Quality.
...
*Why:* Each ratio comes from the platform's own ranking signal: [X open-source weights](https://github.com/xai-org/x-algorithm) (reply ≈ 27× like), LinkedIn 360Brew, [OpusClip 118k-video study](./skills/pubcraft/SOURCES.md#5-industry-studies-and-benchmarks) for short-video.

**🚫 An anti-slop style guide that actually bites.**
40+ hard-banned words (*delve, tapestry, pivotal, leverage, robust, multifaceted, navigate, unlock,* …). Banned structure: "It's not just X — it's Y." Em-dashes capped at one per 500 words. Mandatory two falsifiable specifics (names, dates, numbers) per 500 words. Self-audit runs before delivery.
*Why:* [Originality.ai, GPTZero, Copyleaks](./skills/pubcraft/SOURCES.md#3-ai-detection-and-style-fingerprinting) flag perplexity + burstiness; "Not X, but Y" appears in ~3% of LLM output vs. <0.1% of human-edited prose.
...
*Why:* Google QRG treats YMYL trust as the dominant E-E-A-T pillar; named credentialed authors gained 15–25% in the [March 2026 core update](./skills/pubcraft/SOURCES.md#1-google--search-quality-ranking-and-updates).
...
*Why:* [Princeton GEO study](./skills/pubcraft/SOURCES.md#2-ai-search--geo--aeo--llmo) (statistics-with-context cited 3–4× more), Sedestral 2026 conversion data, Ahrefs/BrightEdge brand-mention analyses.

**📜 2026-current AI disclosure rules.**
TikTok C2PA. YouTube synthetic-content toggle. Meta auto-labeling. EU AI Act Article 50 (effective Aug 2, 2026, fines up to €15M or 3% global turnover). California SB 942 / AI Transparency Act (effective Jan 1, 2026, covered providers). The JUMBF-stripping reality on LinkedIn / X / Bluesky / Threads cross-posts and the hybrid embed + visible-label + sidecar pattern that survives it. Pubcraft knows which apply per surface and per audience.
*Why:* [EU AI Act Article 50, California SB 942, C2PA Content Bindings spec](./skills/pubcraft/SOURCES.md#6-compliance-and-disclosure), YouTube mandatory disclosure (May 21, 2025), TikTok C2PA auto-labeling (1.3B+ videos since Jan 2025).
...
*Why:* Anthropic's [Agent Skills design pattern](https://docs.anthropic.com/en/docs/agents-and-tools/agent-skills/skills-quickstart). Load only what the task needs.
...
[![⬇ Download pubcraft.skill](https://img.shields.io/badge/⬇_Download-pubcraft.skill-0066FF?style=for-the-badge)](https://github.com/thevrus/pubcraft/releases/latest/download/pubcraft.skill)

1. Click the button above to download `pubcraft.skill` (the latest release artifact, auto-built from each tag).
2. In Claude, open **Customize** → **Skills** → **Upload skill** (also reachable as **Settings → Capabilities → Skills**; Anthropic renames this panel occasionally — if you don't see it, check the `/` command menu for `Skills`).
3. Select the `pubcraft.skill` file and toggle the skill on.
4. Type `/pubcraft` or just describe what you want to publish. The skill activates automatically when you ask Claude for any public-facing content.

> **Heads up:** Claude expects an uploaded skill to be a ZIP that contains a *folder* with `SKILL.md` inside (not `SKILL.md` at the root). The released `.skill` is shaped correctly. If you build your own ZIP from a clone, zip the `skills/pubcraft/` folder, not its contents.

**Manual install** (if your network blocks GitHub releases or you want to modify the skill first):

1. Clone the repo and run `npm run build` to produce `dist/pubcraft.zip`.
2. Or download the source as a ZIP, then re-zip `skills/pubcraft/` on its own.
3. Upload that ZIP via the same **Upload skill** dialog.
...
Ask: *"Write a 200-word LinkedIn post about a marketing experiment that flopped."* If loaded, Claude asks a clarifying question (audience, vertical) and refuses to write "Excited to share…" A generic response means the skill isn't loaded; check the install path.
